...
const seedData = {
  employees: [
    { employeeCode: "00000001", name: "管理者", role: "admin" },
  ],
  drinks: [
...
```json
{
  "data": [
    {
      "createdAt": "2026-02-13T09:00:00Z",
      "employee": { "name": "管理者", "employeeCode": "00000001" },
      "hasDiff": true,
      "totalDiff": 3,
      "drinkCount": 6,
      "checks": [
...
全ユーザーがアクセス可能な消費統計データを取得する。

**クエリパラメータ:**
| パラメータ | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| days | number | No | 集計期間（日数、デフォルト: 30、最大: 90） |

**レスポンス（200）:**
```json
{
  "period": { "days": 30, "from": "2026-01-14T00:00:00Z", "to": "2026-02-13T..." },
  "employeeRanking": [
    { "id": 1, "name": "管理者", "employeeCode": "00000001", "totalQuantity": 25 }
  ],
  "drinkBreakdown": [
    { "id": 1, "name": "お茶（緑茶）", "totalQuantity": 40 }
  ],
  "dailyTrend": [
    { "date": "2026-02-07", "quantity": 15 }
  ],
  "stockDepletion": [
    { "id": 1, "name": "お茶（緑茶）", "currentStock": 8, "avgDailyConsumption": 2.5, "daysUntilEmpty": 3 }
  ],
  "drinkDailyTrend": {
    "お茶（緑茶）": [{ "date": "2026-02-07", "quantity": 5 }]
  }
}
```
...
| 設定項目 | 入力形式 | 説明 |
|---------|---------|------|
| Google Chat Webhook URL | テキスト入力 | 差分アラート・在庫低下通知先 |
| 在庫低下通知閾値（本） | 数値入力 | この本数以下になると通知（デフォルト: 3） |
...
全ユーザーがアクセス可能な消費統計画面。メイン画面ヘッダーの「統計」ボタンからアクセス。

**表示内容:**
- 期間選択（7日/14日/30日/60日）
- 在庫消費ペーステーブル（現在庫、1日平均消費、在庫切れまでの日数）
- 日別消費推移（棒グラフ）
- ドリンク別消費割合（円グラフ）
- ドリンク別消費数（横棒グラフ）
- ドリンク別消費推移（上位5種、折れ線グラフ）
- 社員別消費ランキング（横棒グラフ）
...
棚卸し差分アラートおよび在庫低下通知にGoogle Chat Webhookを使用する。

**通知トリガー:**
- 棚卸し実施時に理論在庫と実在庫の差分が1つ以上ある場合
- ドリンク取り出し時に在庫が設定閾値（`low_stock_threshold`）以下になった場合
...
ドリンク取り出し記録時に、在庫が閾値以下になった場合にWebhookで通知する。

**通知条件:**
- `low_stock_threshold` 設定値が1以上
- 取り出し後の在庫数が閾値以下
- 返却時は通知しない
...
在庫が0になった場合は `🚨` アイコンで「*在庫切れ*」と表示。

**実装:**
- `POST /api/transactions` のレスポンス返却後に非同期で実行（レスポンス速度に影響なし）
