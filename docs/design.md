...
在庫数が0未満になる記録は、APIバリデーションで拒否する。棚卸し確定時は、棚卸し記録の作成と在庫更新をトランザクションで一括実行する。
...
- テンキー: 1〜9を横1行に配置。在庫数を超えるボタンは無効化。選択中の数量はハイライト表示
- お客様名テキスト入力（任意）
...
- 在庫0のドリンクはグレーアウトし選択不可
- テンキーの数字を1タップで数量選択（+/-ボタンは廃止）
