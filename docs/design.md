...
| イベント | 計算 |
|---------|------|
| 取り出し記録（Transaction作成） | `stock -= quantity` |
| 入庫登録（StockEntry作成） | `stock += quantity` |
| 棚卸し差分修正（管理者操作） | `stock = actualStock`（管理者の手動修正時） |

在庫数が0未満になる記録は、APIバリデーションで拒否する。
...
棚卸しを実施する。全ドリンク種類の実在庫を一括で登録する。差分がある場合はGoogle Chatに通知を送信する。
...
- `DrinkTile` - ドリンク選択タイル（画像、名称、在庫数を表示）
- `QuantitySelector` - 数量選択（+/- ボタン、デフォルト1、最大値は現在庫数）
- お客様名テキスト入力（任意）
- 確定ボタン（ドリンク未選択時はdisabled）

**動作仕様:**
- ページ表示時にGET /api/drinksでドリンク一覧を取得
- タイルタップでドリンクを選択（選択中のタイルはハイライト表示）
- 在庫0のドリンクはタイルをグレーアウトし選択不可
- 確定ボタン押下でPOST /api/transactionsを呼び出し
- 成功時: 記録完了画面（S-03）へ遷移
